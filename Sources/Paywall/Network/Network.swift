...
extension Network {
    ...
    func paywallByIdentifier(identifier: String, completion: @escaping (Result<PaywallResponse, Swift.Error>) -> Void) {
        ...
        if Store.shared.locales.contains(DeviceHelper.shared.locale) {
          ...
          if let shortLocale = DeviceHelper.shared.locale.split(separator: "_").first {
            let shortLocaleString = String(shortLocale)
            if (Store.shared.locales.contains(shortLocaleString)) {
              queryLocale = URLQueryItem(name: "locale", value: shortLocaleString)
            }
          }
        }
        if let queryLocale = queryLocale {
